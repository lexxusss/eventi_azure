...
function getUserId(originalDetectIntentRequest) {
    if (originalDetectIntentRequest) {
        let ODIR = originalDetectIntentRequest;

        if (ODIR &&
            ODIR.payload &&
            ODIR.payload.data &&
            ODIR.payload.data.user
        ) {
            return md5(ODIR.payload.data.user);
        }
    }
    ...
    if (!found.length) {
        ...
        return {
            ...
            source: !debug ? sourceReg : {
                ...
                contexts: contexts,
                all: context.bindings.searchClientCollection
            }
        };
    }
...
module.exports = {
    ...
    stopNotifyClient: stopNotifyClient
};
